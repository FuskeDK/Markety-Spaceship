...
// Origin gating: only accepts requests from ALLOWED_ORIGINS (marketyleadgen.com,
// nordicsolfilm.dk, *.vercel.app). No admin auth required - the clientToken
// acts as the access key for the client's lead form.
// Used by: client landing pages via their embedded lead forms.
import type { VercelRequest, VercelResponse } from "@vercel/node";
import { createClient } from "@supabase/supabase-js";
import { createClickUpTask, LIST_IDS } from "../lib/server/_clickup.js";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { clientToken, name, email, phone, source } = req.body ?? {};
