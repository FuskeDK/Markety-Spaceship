...
7. **Read the bundled reference freely.** When a pattern is unclear, open the in-skill copy at `~/.claude/skills/build-premium-website/reference/full-reference-app.jsx`.
...
## Markety delivery rules (added by Markety)

These sites are built for Markety clients -- the site represents the CLIENT's own business, not Markety. Additional hard rules on top of the reference system:
...
This is a Markety client delivery. The client's name, phone, services, and branding are on the site. Markety is not the subject of any page content.
